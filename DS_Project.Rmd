...
head(flight_new)
y = flight_new$Fare
x = model.matrix( ~ ., data =(select(flight_new,-Fare, -Flight_code, -sector, -Date_of_journey)))
...
classifier_RF = randomForest(x = x_train, y = y_train, ntree = 100)
classifier_RF
...
Source = "Delhi"
Destination = "Mumbai"
Flight_date = as.Date(a) 
days_left = 5
Tck_class = "Economy" 
Stps = "1"
Arival_time = "After 6 AM" 
Departure_time = "Before 6 PM"
Drtn=2

Journey_day = strftime(Flight_date,"%A") 
flight_inpt = data.frame(Journey_day, Airline, Tck_class, Source, Destination, Stps, Arival_time, Departure_time, Drtn, days_left)
inp = model.matrix( ~ ., data = flight_inpt)
prd = predict(optimal_model, inp)
...
head(x_test)
class(x_test)
head(select(flight_new,-Fare, -Flight_code, -sector, -Date_of_journey))
class(flight_new)

flightPricePredict <- function(Airline, Source, Destination, Flight_date, days_left, Tck_class, Stps, Arival_time, Departure_time, Drtn=2){
  #TODO convert to journey day also
  Journey_day = strftime(Flight_date,"%A") 
  flight_inpt = data.frame(Journey_day, Airline, Tck_class, Source, Destination, Stps, Arival_time, Departure_time, Drtn, days_left)
  inp = model.matrix( ~ ., data = flight_inpt)
  prd = predict(optimal_model, inp)
...
a = "11/02/22"
flightPricePredict("SpiceJet", "Delhi", "Mumbai", as.Date(a), 5, "Economy", "1", "After 6 AM", "Before 6 PM")
