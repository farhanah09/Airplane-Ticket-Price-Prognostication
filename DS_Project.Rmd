...
head(flight_new)
y = flight_new$Fare
x = model.matrix( ~ ., data =(select(flight_new,-Fare, -Flight_code, -Source, -Destination, -Date_of_journey, -Duration_in_hours)))
head(x)
#Splitting data
...
#Evaluation metrics
postResample(y_test, predicted)
saveRDS(classifier_RF, file = "model_file_Random_Forest_25.rds")

model <- readRDS("model_file_Random_Forest_25.rds")
predicted = predict(model, x_test)
residuals = predicted - y_test
plot(y_test,residuals)
#Evaluation metrics
